...
分割的“全监督 / 弱监督”是提交者对外部训练条件的声明，随冻结配置保存并进入比较相容性检查。平台不读取训练集或训练日志，两类模型都使用相同冻结、完整标注的测试集评分。
...
分割预览只保存归一化原图与预测遮罩叠加图；配准预览只保存 moving points 与 predicted points。隐藏分割标签 / fixed points 不进入预览 NPZ，也不进入公开聚合报告。每个阶段任务最多保存 6 个私有预览。

## 比较与可追溯性

比较校验基准版本/协议、真实或合成属性、任务顺序、测试资产元数据、客户端版本/数量、分割监督声明、未见任务开关、输出头与指标条件。方法名称、模型还是预测的输入方式不改变测试条件；原始来源仍随报告保存。无最终阶段的对比保留空值，不以其他阶段替代。
