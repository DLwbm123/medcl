...
当前固定入口使用机器名 `med-cl`，完整网址以服务器网页运行目录的 `FIXED_PUBLIC_URL` 为准。下方 `web-display` 名称属于重命名前的历史验收记录。
...
### 普通浏览器验收通过（2026-09-11）

- 后续查询中，114 与阿里公共 DNS 均已返回两条 A 记录。清理 macOS DNS 缓存后，普通域名的直连 HTTPS health 返回 `200 ok`（约 0.84 秒），无需请求级地址指定。
- 本机既有代理路径仍连接超时；代理日志另确认其直接连接分支报 `dns resolve failed: couldn't find ip`。最终只在当前 Ethernet 网络服务的系统代理例外列表中增加该完整域名，保留其余例外和代理设置，使 Chrome 使用已经验证的系统解析及直连路径。排查时尝试的代理规则已移除，未保留额外模式或节点变更；未修改 hosts、写死 IP 或关闭 TLS 证书校验。此例外针对验收电脑当前网络服务，代理软件重新设置系统代理或更换网络服务后需复核。
- 普通 Chrome 已实际打开固定 HTTPS 网址，完整显示首页；全监督 Domain-CL 从 T1 切换到 T2，原始影像与分割结果正常加载，三维浏览显示三个 MPR 和独立预测分割 3D 对象。这也验证了 Streamlit 交互更新及组件资源加载，验收依据包含实际页面而非仅 HTTP 状态。
- 本机实际截图保存在忽略目录 `.local/tunnel/fixed-homepage.png` 和 `.local/tunnel/fixed-four-viewports.png`，不公开含医学图像或个人浏览器信息的截图。未在本轮测试 3D 旋转、手机蜂窝网络、其他访问者网络或容器重启；没有应用代码修改，未重复运行应用单元测试。前述失败记录保留为当时状态，本节记录后续已完成的浏览器验收。

### 固定网址包含项目名称（2026-09-11）

- 按用户要求，将 Tailscale 机器名称改为 `med-cl`，固定网址变为 `https://med-cl.<tailnet>.ts.net/`。更新同一网页的 Funnel 转发与 `FIXED_PUBLIC_URL`，仍指向 `http://127.0.0.1:8501`；原 `web-display` 地址不作为后续展示入口。没有重启网页进程或修改评分配置。
- 使用实际安装版本 1.102.4 的 LocalAPI 更新 Hostname，经标准输入传递名称，使服务器进程命令行继续保持中性；SSH、DNS、路由设置未改变。新域名证书已签发，设备 Running 且 health 无告警。验收电脑已有的精确域名代理例外同步替换为新名称。
- 公共 DNS 返回新域名的两条 A 记录，普通域名直连 HTTPS health 返回 `200 ok`（约 2.98 秒）。普通 Chrome 已完整打开新网址首页，截图 `.local/tunnel/med-cl-homepage.png` 保存在本机忽略目录。没有重复三维专项测试或独立蜂窝网络验证；本次仅重命名入口并更新部署记录，未修改应用代码。
