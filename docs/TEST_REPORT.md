...
结论：页面已改为顶部导航和“三类大任务 → 增量场景 → 监督方式 → 具体协议”的纵向层级。分割已接入真实六中心平台协议、原图/预测叠加图和病例 Dice；分类显示类别名称和逻辑客户端联邦评分；配准已有合成点位图与 TRE。**全部验收模型均为未训练工程基线，不是原论文复现、模型有效性或临床结论。**
...
- 分割全/弱监督声明的冻结和比较门；分割/配准私有预览文件键及相对路径检查，确认不包含 target / fixed 真值数组。
...
上述真实记录只提交最终阶段，BWT、遗忘、FWT 与 BWTR 均不可计算。真实指标来自本轮 scorer，不是从论文、旧日志或合成数据移植。新版本的完整**全局聚合**证据见 [acceptance_summary_v2.json](acceptance_summary_v2.json)；私有病例预览不进入公开聚合文件。
...
待接入：MMWHS/多器官全局标签和病例协议；真实配准点对/模型/坐标约定；研究模型的审核结构或合规预测。尚未完成真实配准、原论文 FedSubMerge/SAMCL 复现、第三方训练模型验证、公网部署或安全认证。
